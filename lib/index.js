      ...
      _.each(fields, function(field, i){
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if (defaultVal === null) {
				if (!_tables[table][field].allowNull && 'createdAt,updatedAt'.indexOf(field) !== -1){
					text[table] += spaces + spaces + spaces + attr + ": sequelize.literal('CURRENT_TIMESTAMP')";
				}
              return true;
            } else {
              text[table] += spaces + spaces + spaces + attr + ": " + SqlString.escape(_.trim(val_text, '"'), null, self.options.dialect);
            }
          }
