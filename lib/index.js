...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    ...
    if (self.options.dialect === 'mssql') {
      schema = table.schema;
      table = table.tableName;
    }

    self.queryInterface.describeTable(table, schema).then(function(fields) {
      self.tables[table] = fields;
      // Inject the tables schema if MSSQL and it's not dbo (the default schema)
      if (self.options.dialect === 'mssql' && schema !== 'dbo') self.tables[table].__schemaName__ = schema;
      _callback();
    }, _callback);
  }

  if ((self.options.dialect === 'postgres' || self.options.dialect === 'mssql') && self.options.schema) {
  ...
  function processTables(__tables) {
    ...
    const comparer = function(a, b) {
      if (self.options.dialect === 'mssql') return (a.tableName === b);
      return a === b;
    };

    if      (self.options.tables)     tables = _.intersectionWith(__tables, self.options.tables, comparer)
    else if (self.options.skipTables) tables = _.differenceWith  (__tables, self.options.skipTables, comparer)
  ...
  function mapForeignKeys(table, fn) {
    ...
    var schema = self.sequelize.config.database;
    ...
    const sql = self.dialect.getForeignKeysQuery(table, schema);
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback) {
      // Grab the table schema if needed and then remove it from the list of columns
      const schema = self.tables[table].__schemaName__;
      delete self.tables[table].__schemaName__;
      ...
      _.each(fields, function(field, i) {
        ...
        _.each(fieldAttr, function(attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (self.options.dialect === "mssql" &&  defaultVal && defaultVal.toLowerCase() === '(newid())') {
            ...
            // mssql bit fix
            else if (self.options.dialect === "mssql" && self.tables[table][field].type.toLowerCase() === "bit") {
      ...
      // Add the tables schema if there is one
      if (schema !== undefined) text[table] += spaces + spaces  + "schema: '" + schema + "',\n";
