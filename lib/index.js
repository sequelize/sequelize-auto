    ...
    .then(function(tables){
      ...
      async.each(tables, function(table, _callback){
        ...
        async.each(tableNames, function(table, _callback){
          ...
          fields.forEach(function(field, i){
            ...
            if (Sequelize.Utils._.isString(defaultVal) && defaultVal.toLowerCase().indexOf('nextval') !== -1 && defaultVal.toLowerCase().indexOf('regclass') !== -1) {
              ...
              fieldAttr.forEach(function(attr, x){
                ...
                if (attr === "special" || attr === "primaryKey") {
                  return true;
                }
          ...
          //conditionally add aditional options to tag on to orm objects
          var hasAditional = Object.keys(options.aditional).length >0;
          if(hasAditional) {
            
            text[table]  += ", {\n";
            for(var aditional in options.aditional) {
              text[table] += spaces + spaces + aditional + ": " + options.aditional[aditional] + ",\n";
            }
