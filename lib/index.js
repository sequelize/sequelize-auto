        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
             ...
             if (_tables[table][field][attr] === true && (! _tables[table][field].hasOwnProperty('foreignKey') || (_tables[table][field].hasOwnProperty('foreignKey') && !!_tables[table][field].foreignKey.isPrimaryKey)))
...
AutoSequelize.prototype.write = function(attributes, callback) {
  ...
  mkdirp.sync(self.options.directory)

  async.each(tables, createFile, callback)

  function createFile(table, _callback){
    fs.writeFile(path.join(self.options.directory, table + '.js'), attributes[table], _callback);
  }
}
