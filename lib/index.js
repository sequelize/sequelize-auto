...
AutoSequelize.prototype.getAliasForTable = function (table) {
  var self = this;

  var aliasObject = _.find(self.sequelize.options.aliases, { 'table':  table } );

  if (aliasObject === null || aliasObject === undefined )
    return table;
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      text[table] += spaces + "return sequelize.define('" + self.getAliasForTable(table) + "', {\n";
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              ...
              text[table] += spaces + spaces + spaces + spaces + "model: \'" + self.getAliasForTable(self.tables[table][field][attr].target_table) + "\',\n"
...
AutoSequelize.prototype.write = function(attributes, callback) {
  ...
  function createFile(table, _callback){
    fs.writeFile(path.resolve(path.join(self.options.directory, self.getAliasForTable(table) + '.js')), attributes[table], _callback);
  }
}
