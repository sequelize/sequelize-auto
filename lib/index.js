...
AutoSequelize.prototype.build = function(callback) {
  ...
  this.queryInterface.showAllTables().then(function (__tables) {
    ...
    tables = self.options.tables ? _.intersection(__tables, self.options.tables) : __tables;
  ...
  function mapForeignKeys(table, fn) {
    ...
    function assignColumnDetails(ref) {
      ...
      var isPrimaryKey = _.isFunction(self.dialect.isPrimaryKey) && self.dialect.isPrimaryKey(ref);
      ...
      if (isForeignKey || isPrimaryKey) {
        ...
        self.foreignKeys[table][sourceColumn] = ref
      }
    }
  }
}
