        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if(defaultVal === null || defaultVal === undefined) {
              ...
              var do_escape_text = _.isString(val_text) && !val_text.match(/^sequelize\.[^(]+\(.*\)$/);
              if (field_type.indexOf('uuid') === 0 || field.type.indexOf('uniqueidentifier') === 0) {
                if (defaultVal.toLowerCase().indexOf('uuidv4') === 0) {
                  val_text = "DataTypes.UUIDV4"
                  do_escape_text = false;
                } else if (defaultVal.toLowerCase().indexOf('uuidv1') === 0) {
                  val_text = "DataTypes.UUIDV1"
                  do_escape_text = false;
                } else {
                  val_text = quoteWrapper + val_text + quoteWrapper
                }
              } 
              val_text = do_escape_text ? SqlString.escape(_.trim(val_text, '"'), null, self.options.dialect) : val_text;
