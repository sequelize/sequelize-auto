...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback) {
    self.queryInterface.describeTable(table, self.options.schema).then(function(fields) {
      ...
      self.sequelize.query('SHOW INDEX FROM `' + table + '`', {
        raw: true,
        type: self.sequelize.QueryTypes.SHOW
      }).then(res => {
        this.indexes = res;
        _callback();
      })
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function caseTableName(name) {
    if (!/[\_-]/.test(name))
      return name.replace(/^(.)/g, g => g.toUpperCase())
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback) {
      ...
      text[table] += spaces + "return sequelize.define('" + caseTableName(tableName) + "', {\n";
          ...
          if (attr === "foreignKey") {
            ...
            if (_.isString(defaultVal)) {
              ...
              if (_.endsWith(defaultVal, '()')) {
                ...
                } else if (_.includes(['current_date', 'current_time', 'localtime', 'localtimestamp'], defaultVal.toLowerCase())) {
            ...
            if (defaultVal === null || defaultVal === undefined) {
              ...
              val_text = _.isString(val_text) && !val_text.match(/^sequelize\.[A-Za-z]+$/) ? SqlString.escape(_.trim(val_text, '"'), null, self.options.dialect) : val_text;
            ...
            if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
              ...
              val = "DataTypes.TEXT('tiny')";
            } else if (_attr.match(/^mediumtext$/)) {
              val = "DataTypes.TEXT('medium')";
      ...
      if (indexes.length) {
        let t = spaces + spaces + "indexes: [\n";
        indexes[0].forEach(i => {
          const key = i.Key_name
          const column = i.Column_name
          const type = i.Index_type
          const isUnique = i.Non_unique === 0
          t += spaces + spaces + spaces + "{ "
          t += "name: '" + key + "', "
          t += "fields: [ '" + column + "' ], "
          t += "unique: " + (isUnique ? 'true' : 'false') + ' '
          t += "},\n"
        })
