...
AutoSequelize.prototype.write = function(attributes, callback) {
  ...
  function createFile(table, _callback){
    ...
    if (prefixes) {
      regexPattern = Array.isArray(prefixes)
        ? prefixes.join('|')
        : prefixes;

      _table = table.replace(new RegExp(regexPattern), '');
    }
    fs.writeFile(path.resolve(path.join(self.options.directory, (_table ? _table : table) + '.js')), attributes[table], _callback);
  }
}
