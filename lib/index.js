...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    self.sequelize.query('SHOW FULL COLUMNS FROM `' + table + '`', {type: 'SELECT'}).then(function (rawData) {
      var fields = {}
      rawData.forEach(function(res) {
        fields[res.Field] = {
          type: res.Type.toUpperCase(),
          allowNull: (res.Null === 'YES'),
          defaultValue: res.Default,
          primaryKey: res.Key === 'PRI',
          comment: res.Comment
        }
      })
