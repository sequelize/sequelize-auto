...
function generateIndexExplorer(tableNames, isSpaces, indentation, isCamelCase, isCamelCaseForFile) {
  ...
  fileTextOutput += "module.exports = function getModels(sequelize) {\n";
  fileTextOutput += spaces + "const models = {\n";
  ...
  `;
  fileTextOutput += spaces + "return models;\n";
  fileTextOutput += "};\n";
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      if (!self.options.typescript) {
        ...
        if (self.options.isEgg) {
          text[table] += `module.exports = app => {\n${spaces}const DataTypes = app.Sequelize;\n`;
        } else {
          text[table] += "module.exports = function(sequelize, DataTypes) {\n";
        }
        text[table] += spaces + `const Model = ${self.options.isEgg ? 'app.model' : 'sequelize'}.define('${tableName}', {\n`;
      } else {
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (self.tables[table][field][attr])
              text[table] += spaces + spaces + spaces + `comment: '${self.tables[table][field][attr]}'`;
            else return true;
      ...
      const hasadditional = _.isObject(self.options.additional) && _.keys(self.options.additional).length > 0;
      ...
      text[table] += `);\n${spaces}Model.associate = function (models) {\n${spaces}//  Model.hasMany(${self.options.isEgg ? 'app.model.xx' : 'models.xx'}, {sourceKey: "xx", foreignKey: "xx"});\n${spaces}};\n${spaces}return ${tableName};\n};\n`;
      _callback(null);
    }, function () {
      ...
      if (!self.options.typescript) indexExplorers = generateIndexExplorer(_.keys(text), self.options.spaces, self.options.indentation, self.options.camelCase, self.options.camelCaseForFileName);
      
      if (self.options.directory) {
        return self.write(text, typescriptFiles, callback, indexExplorers);
      }
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback, indexExplorers) {
  ...
  } else if(!self.options.isEgg) {
    fs.writeFileSync(path.join(self.options.directory, "index.js"), indexExplorers, "utf8");
  }
