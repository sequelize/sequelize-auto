...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    ...
    async.each(tables, compose([mapForeignKeys, mapIndexes, mapTable]), callback);
  }
  ...
  function mapIndexes(table, fn) {
    if (!self.dialect) return fn()

    var sql = self.dialect.getIndexesQuery(table, self.sequelize.config.database)
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if (self.indexes[table] && self.indexes[table].length > 0) {
        text[table] += spaces + spaces + "indexes: [\n";
        _.each(self.indexes[table], function (index) {
          text[table] += spaces + spaces + spaces + "{\n";
          text[table] += spaces + spaces + spaces + spaces + "name: '" + index.index_name + "',\n";
          if (index.index_type === 'FULLTEXT' || index.index_type === 'SPATIAL') {
            text[table] += spaces + spaces + spaces + spaces + "type: '" + index.index_type + "',\n";
          } else {
            text[table] += spaces + spaces + spaces + spaces + "method: '" + index.index_type + "',\n";
          }
          text[table] += spaces + spaces + spaces + spaces + "fields: " + JSON.stringify(index.index_fields.split(',')) + "\n";
          text[table] += spaces + spaces + spaces + "},\n";
        })
        // removes the last `,` within the attribute options
        text[table] = text[table].trim().replace(/,+$/, '') + "\n";
