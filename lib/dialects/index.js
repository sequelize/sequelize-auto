...
exports.mssql = {
   ...
   */
  getForeignKeysQuery: function (tableName, schemaName) {
    const schemaQueryAddOn = (schemaName !== undefined) ? ` AND ccu.TABLE_SCHEMA = ${sequelize.Utils.addTicks(schemaName, "'")}` : '';
    const sql = `
      SELECT
      ...
      WHERE ccu.table_name = ${sequelize.Utils.addTicks(tableName, "'")}${schemaQueryAddOn}`;
    return sql;
  },
  /**
   * Determines if record entry from the getForeignKeysQuery
   ...
   */
  isSerialKey: function (record) {
    return _.isObject(record) && exports.mssql.isPrimaryKey(record) && (_.has(record, 'isSerialKey') &&
      record.isSerialKey);
  },
  /**
   * Overwrites Sequelize's native method for showing all tables.
   ...
   */
  showTablesQuery: function(schema) {
    return "SELECT TABLE_NAME, TABLE_SCHEMA FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '" + schema + "';";
  }
}
