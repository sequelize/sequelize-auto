...
exports.postgres = {
  ...
  getTablesInfoQuery: function() {
    return `SELECT C.oid,C.relname AS tablename,obj_description ( C.oid ) AS COMMENT FROM pg_class C LEFT JOIN pg_namespace n ON n.oid = C.relnamespace
            WHERE ( ( C.relkind = 'r' :: "char" ) OR ( C.relkind = 'f' :: "char" ) OR ( C.relkind = 'p' :: "char" ) )
            AND n.nspname = 'public' ORDER BY tablename`;
  }
};
