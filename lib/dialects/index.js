...
exports.sqlite = {
  ...
  getIndexesQuery: function (tableName, schemaName) {
    return `SELECT * sqlite_master WHERE type = 'index' and tbl_name = '${tableName}';`;
  },
  /**
...
exports.mysql = {
  ...
  getIndexesQuery: function (tableName, schemaName) {
    return `SELECT * FROM 
      (SELECT
        index_name,
        group_concat(column_name order by seq_in_index separator ',') as index_fields,
      ...
      WHERE
        ...
        AND NON_UNIQUE = 1
      GROUP BY index_name) as a
    WHERE NOT EXISTS
      (SELECT * FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
      WHERE
        TABLE_SCHEMA = '${schemaName}'
        AND TABLE_NAME = '${tableName}'
        AND COLUMN_NAME = a.index_fields
...
exports.mssql = {
   ...
   */
  getIndexesQuery: function (tableName, schemaName) {
    return `SELECT a.name AS index_name,
      d.name AS index_column_name,
      d.colid AS index_position,
      FROM sysindexes a
