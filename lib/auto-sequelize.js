...
const { mapType, autoBindAssociate, generateIndexExplorer, getKeyName, replaceText } = require("./utils");
...
class AutoSequelize {
  ...
  _map2Attributes(table) {
    ...
    if (!this.hasPk(table) && !this.tables[table].id) {
  ...
  _generateAttrText(table) {
    ...
    return fields.reduce((memo, field, i) => {
      ...
      memo += fieldAttr.reduce((str, attr) => {
        ...
        if (attr === "foreignKey") {
          if (isSerialKey) {
            ...
            str += `references: {model: "${this._getModelName(attrValue.foreignSources.target_table)}",key: '${this.options.camelCase ? _.camelCase(attrValue.foreignSources.target_column) : attrValue.foreignSources.target_column}'}`;
          } else return str;
          ...
          if (attrValue)
            str += `comment: '${replaceText(attrValue)}'`;
          else
            return str;
      ...
      // NOTE: 为防止sequelize报错 自动添加id字段为主键
  ...
  _generateOptionText(table) {
    ...
    if (this.tablesComments[table]) {
      optionText += `comment: '${replaceText(this.tablesComments[table])}',`;
    }
    if (hasAdditional) {
...
const Model = ${this.options.isEgg ? "app.model" : "sequelize"}.define('${modelName}', `;
  ...
  _generateTypesText() {
    ...
    ${!this.options.typescript ? "export default function getModels(seq:Sequelize):ITables" : ""}
    `;
  }
  ...
  _write(attributes, typescriptFiles, indexExplorers) {
    ...
    return Promise.all([...tables.map(table => {
      ...
      if (this.options.typescript) {
        ...
        return Promise.all([
          this._writeFile(path.join(this.options.directory, "index.js"), indexExplorers),
          this._writeFile(path.join(this.options.directory, "index.d.ts"), this.typesText)
        ]);
      }
    })()]);
  }
