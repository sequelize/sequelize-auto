...
function mapType(attrValue, isStr, {mapInt2Bool = true} = {}) {
  ...
  if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
    ...
    type = getLengthType(_attr, "DOUBLE", isStr)
    ...
    type = isStr ? "new DataTypes.BLOB('long')" : DataTypes.BLOB('long');
  } else if (_attr.match(/^mediumblob/)) {
    type = isStr ? "new DataTypes.BLOB('medium')" : DataTypes.BLOB('medium');
  } else if (_attr.match(/^tinyblob/)) {
    type = isStr ? "new DataTypes.BLOB('tiny')" : DataTypes.BLOB('tiny');
  } else if (_attr.match(/^blob/)) {
    type = isStr ? "new DataTypes.BLOB" : DataTypes.BLOB;
