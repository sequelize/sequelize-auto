    ...
    if (isMiddleTable(currentModel)) {
      referencesFields.forEach(key => {
        ...
        referencesFields.forEach(field => {
          ...
          if (key === field || source === target) return;
          source.belongsToMany(target, {
            through: currentModel,
            foreignKey: key,
            sourceKey: sourceReferences.key,
            targetKey: targetReferences.key
          });
...
`;
  for (let i = 0; i < tableNames.length; i++) {
    ...
    fileTextOutput += getKeyName(tableForClass) + ": sequelize.import(path.join(__dirname, './" + tableForFile + "')),\n";
  }
  fileTextOutput += `  };
