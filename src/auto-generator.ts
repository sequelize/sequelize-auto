...
export class AutoGenerator {
    ...
    tableNames.forEach(table => {
      ...
      if (this.options.lang === 'ts') {
        ...
        needed.forEach(fkTable => {
          ...
          const filename = recase(this.options.caseFile, fkTable, this.options.singularize);
          str += 'import type { ';
  ...
  private addTypeScriptAssociationMixins(table: string): Record<string, any> {
      ...
      fkFieldNames.forEach(fkFieldName => {
        ...
        if (spec.isForeignKey) {
          if ((!mySchemaName || spec.source_schema === mySchemaName) && spec.source_table === myTableName) {
            const fkTable = spec.foreignSources.target_table as string;
            const btModel = recase(this.options.caseModel, fkTable, this.options.singularize);
            ...
            needed[fkTable] ??= new Set();
            needed[fkTable].add(btModel);
            needed[fkTable].add(btModel + 'Id');
            ...
            const fkTable = spec.foreignSources.source_table as string;
            const hasModel = recase(this.options.caseModel, fkTable, this.options.singularize);
            ...
            if (isOne) {
              ...
              needed[fkTable].add(hasModel);
              needed[fkTable].add(`${hasModel}Id`);
              needed[fkTable].add(`${hasModel}CreationAttributes`);
            } else {
              const hasModelSingular = Utils.singularize(hasModel);
          ...
          if (spec.isPrimaryKey && (!mySchemaName || spec.foreignSources.target_schema === mySchemaName) &&
            ...
            if (otherKey) {
              const fkTable = otherKey.foreignSources.target_table as string;
              const otherModel = recase(this.options.caseModel, fkTable, this.options.singularize);
              ...
              needed[fkTable] ??= new Set();
              needed[fkTable].add(otherModel);
              needed[fkTable].add(`${otherModel}Id`);
            }
          }
    ...
    if (needed[myTableName as string]) {
      delete needed[myTableName as string]; // don't add import for self
    }
    return { needed, str };
  }
