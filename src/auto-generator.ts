...
export class AutoGenerator {
  ...
  makeHeaderTemplate({ importSequelize }: { importSequelize?: boolean } = {}) {
    let header = '';
  ...
  generateText() {
    ...
    tableNames.forEach(table => {
      ...
      str = this.makeHeaderTemplate({ importSequelize: str.includes('Sequelize') }) + str;
